<template>
    <view class="easy-loadimage" :id="uid" @tap="onTap">
    ...
    methods: {
      ...
      onScroll (scrollTop) {
        ...
        query.select('#' + id).boundingClientRect(data => {
          if (!data) return
          // console.log(data, this.viewHeight)
...
<style scoped>
    ...
    .loadfail-img {
        height: 100%;
        background: url('~@/static/imgs/load_failed.png') no-repeat center;
        background-size: 50%;
    }
    ...
    /* 转圈 */
    .spin-circle {
        background: url('~@/static/imgs/loading.gif') no-repeat center;
